...
void regHandle(int serverSocket, char *handle)
{
	...
	// printf("Socket %d: Clientname: %.*s\n", serverSocket, handleLen, &buffer[2]);	// DEBUGGING ONLY
}
...
void processMsgFromServer(int serverSocket)
{
	...
	case 6: // message or broadcast or multicast packet
	{
		...
		if (flag == 4)
			msgStartIdx = getMsgIdx(buffer, 1); // broadcast packet
		else
			msgStartIdx = getMsgIdx(buffer, 0);
	...
	{
		char badHandle[MAX_HANDLE_LENGTH + 1] = {0};
...
void processStdin(int serverSocket)
{
	uint8_t userCmd[MAXBUF] = {0}; // data buffer
	...
	// parse the user cmd
	uint8_t flag = 0;
	char destHandles[MAX_DEST_HANDLES][MAX_HANDLE_LENGTH + 1] = {0};
	int numDestHandles = 0;
	char messageText[MAX_MESSAGE_LENGTH + 1] = {0};
...
int getMsgIdx(uint8_t *buffer, int broadcast)
{
	...
	if (broadcast)
		return idx; // broadcast packet no dest handles
