...
void talkToServer(int socketNum, struct sockaddr_in6 * server)
{
	...
	while(1)
	{
		...
		int pduLen = createPDU(pduBuffer, seqNum, 0x9, (uint8_t *)input, inputLen);
		...
		// send PDU (filename packet)
		safeSendto(socketNum, pduBuffer, pduLen, 0, (struct sockaddr *) server, serverAddrLen);

		// poll for PDU (10 timeouts then exit)
		int sockReady = pollCall(1000);	// 1 sec timeout
		if (sockReady < 0)
